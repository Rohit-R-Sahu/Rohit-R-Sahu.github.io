...
function CodeSphere({ theme }: { theme: string }) {
  ...
  const material = new THREE.MeshStandardMaterial({
    color: theme === 'dark' ? new THREE.Color('#6172f9') : new THREE.Color('#4249ef'),
    metalness: 0.3,
    roughness: 0.4,
    wireframe: true
  });
    ...
    const typeText = () => {
      ...
      if (!isDeleteMode.current) {
        ...
        if (charIndexRef.current < currentText.length) {
          if (textRef.current) {
            textRef.current.textContent = currentText.substring(0, charIndexRef.current + 1);
          }
        ...
        if (charIndexRef.current > 0) {
          ...
          currentTextRef.current = (currentTextRef.current + 1) % TYPING_TEXTS.length;
        ...
        >
          ...
          <div className="h-14 flex justify-center items-center">
            <h2 className="text-xl sm:text-2xl md:text-3xl font-medium mb-8">
              ...
              <span className="animate-blink text-indigo-600 dark:text-indigo-400">|</span>
            </h2>
          </div>
          
          <p className="text-lg sm:text-xl text-gray-700 dark:text-gray-300 mb-10 max-w-3xl mx-auto">
            With over 3 years of experience building robust applications using 
            <span className="font-semibold text-indigo-600 dark:text-indigo-400"> MERN</span>, 
            <span className="font-semibold text-indigo-600 dark:text-indigo-400"> Java/Spring</span>, and 
            <span className="font-semibold text-indigo-600 dark:text-indigo-400"> Python</span>. 
